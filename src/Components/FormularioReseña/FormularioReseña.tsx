...
export const FormularioResena: React.FC<Props> = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5173/usuario/${usuario_id}`)
      .then(res => {
        if (res.data?.resena) {
  ...
  const enviarResena = async () => {
    ...
    try {
      await axios.post('http://localhost:5173/resena/agregar', {
        resena: mensaje,
        fecha_resena,
  ...
  const editarResena = async () => {
    ...
    const fecha_resena = new Date().toISOString().slice(0, 19).replace('T', ' ');
    try {
      await axios.put('http://localhost:5173/resena/editar', {
        resena: mensaje,
        fecha_resena,
        id_usuario: usuario_id
      });
  ...
  const eliminarResena = async () => {
    if (!usuario_id) return alert('Inicia sesión nuevamente.');
    try {
      await axios.delete('http://localhost:5173/resena/eliminar', {
