...
const PaymentBrick = React.memo(() => {
  ...
  useEffect(() => {
    const fetchPreferenceId = async () => {
      try {
        const response = await fetch("http://localhost:5173/api/create_preference", {
          method: "POST",
      ...
      await bricksBuilder.create("payment", "payment-brick-container", {
        ...
        callbacks: {
          ...
          onSubmit: async ({ selectedPaymentMethod, formData }) => {
            ...
            const response = await fetch("http://localhost:5173/api/process_payment", {
              method: "POST",
              headers: {
          ...
          onPaymentApproved: async (payment) => {
            ...
            fetch("http://localhost:5173/api/process_payment", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
