...
const [mape, setMape] = useState<number | null>(null);
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`http://localhost:5173/metricas/ventas?agrupacion=${agrupacion}`);
...
,
  ...
  try {
    const res = await fetch(`http://localhost:5173/metricasIA/metricas/prediccion?agrupacion=${agrupacion}`);
    if (!res.ok) {
   ...
   const formatYAxisTick = (value: number) => {
        ...
        const cargarTopProductos = async () => {
            try {
                const [masRes, menosRes] = await Promise.all([
                    axios.get('http://localhost:5173/metricas/top-productos?tipo=mas&limite=5'),
                    axios.get('http://localhost:5173/metricas/top-productos?tipo=menos&limite=5')
                ]);
    ...
    useEffect(() => {
        const cargarInactivos = async () => {
            try {
                const res = await axios.get('http://localhost:5173/metricas/productos-inactivos');
